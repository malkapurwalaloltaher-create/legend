...
module.exports = {
  ...
  usage: "$setstaffrole @role1, @role2",

  async executePrefix(message, args) {
    if (!message.guild) return;
    if (!canManageRoles(message.member)) {
      const reply = await message.reply("❌ Only server admins/managers can add staff roles.").catch(() => null);
    ...
    if (!roleIds.length) {
      const reply = await message.reply("❌ Please mention one or more roles. Usage: `$setstaffrole @role1, @role2`").catch(() => null);
      if (reply) setTimeout(() => reply.delete().catch(() => null), 9000);
      return;
    }

    const config = await addRoleIds(message.guild, "staff", roleIds, message.author.id);
