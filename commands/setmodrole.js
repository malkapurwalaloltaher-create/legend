...
const { addRoleIds, formatRoleList } = require("../utils/roleAccess");
...
module.exports = {
  ...
  usage: "$setmodrole @role1, @role2",

  async executePrefix(message, args) {
    if (!message.guild) return;
    if (!canManageRoles(message.member)) {
      const reply = await message.reply("❌ Only server admins/managers can add mod roles.").catch(() => null);
    ...
    if (!roleIds.length) {
      const reply = await message.reply("❌ Please mention one or more roles. Usage: `$setmodrole @role1, @role2`").catch(() => null);
      if (reply) setTimeout(() => reply.delete().catch(() => null), 9000);
      return;
    }

    const config = await addRoleIds(message.guild, "mod", roleIds, message.author.id);

    const embed = new EmbedBuilder()
      .setColor(0x8b5cf6)
      .setTitle("✅ Mod Roles Added")
